...
import {fromRanges, intersectLine, pointSetToAngle, toRadian, vecAdd} from '../utils/math';
import {getFormSize, getPolygonEdges} from "./makeGeneratedPattern";
import {reflectAtEdge} from "../rough/hachure";


export function makeAlSamad(angle?: number): Pattern {
  const mod = fromRanges([[-6, 6]], angle);   // some mods seem to cause trouble, such as 6
  ...
  return {
    ...
    externalShapes: [
      reflectAtEdge([b, p, y2, y3, q, c], [c, b]),
      reflectAtEdge([c, r, y3, y4, s, d], [c, d]),
      reflectAtEdge([e, u, y5, y4, t, d], [e, d]),
    ],
    fillPatterns: [
      [[
        [[0, 1, 2, 3, 4, 5, 7, 8, 9,]],
